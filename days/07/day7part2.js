...
for (let i = 0; i < data.length; i++) {
    ...
    let whileI=0;
    while (whileI < 3**(numArrLen-1) && allLoopExit) {
        let operators=[whileI.toString(3).padStart(numArrLen-1,'0').replaceAll('0','+').replaceAll('1','*').replaceAll('2','|')];
        let whileJ = 0;
        while ( whileJ < operators.length) {
            ...
            while (whileK < operators[whileJ].length && allLoopExit) {
                ...
                } else if (operators[whileJ][whileK]==='*') {
                    equationCounter=equationCounter*numArr[whileK+1];
                } else {
                    equationCounter=Number(equationCounter+''+numArr[whileK+1]);
                }
                whileK++;
            }
